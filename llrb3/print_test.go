...
func TestPut(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		tree.Put(key, StringValue{value})
...
func TestInsertAfter(t *testing.T) {
	...
	// 10번의 삽입 수행
	var nodes []*Node[StringKey, StringValue]
	for i := 0; i < 10; i++ {
		key := StringKey(fmt.Sprintf("%d", i))
		value := fmt.Sprintf("값-%d", i)
		newNode := NewNode(key, StringValue{value}, true)
	...
	newNode10 := tree.InsertAfter(node3, NewNode(StringKey("10"), StringValue{"값-10"}, true))
	nodes = append(nodes, newNode10)
	fmt.Println("\n=== 3번 노드 뒤에 10번 노드 삽입 ===")
	fmt.Println(tree.Print())

	newNode11 := tree.InsertAfter(node7, NewNode(StringKey("11"), StringValue{"값-11"}, true))
	nodes = append(nodes, newNode11)
	fmt.Println("\n=== 7번 노드 뒤에 11번 노드 삽입 ===")
	fmt.Println(tree.Print())

	newNode12 := tree.InsertAfter(nodes[10], NewNode(StringKey("12"), StringValue{"값-12"}, true))
	nodes = append(nodes, newNode12)
	fmt.Println("\n=== 10번 노드 뒤에 12번 노드 삽입 ===")
	fmt.Println(tree.Print())

	newNode13 := tree.InsertAfter(node1, NewNode(StringKey("13"), StringValue{"값-13"}, true))
	nodes = append(nodes, newNode13)
	fmt.Println("\n=== 1번 노드 뒤에 13번 노드 삽입 ===")
	fmt.Println(tree.Print())

	newNode14 := tree.InsertAfter(node6, NewNode(StringKey("14"), StringValue{"값-14"}, true))
	nodes = append(nodes, newNode14)
	fmt.Println("\n=== 6번 노드 뒤에 14번 노드 삽입 ===")
	fmt.Println(tree.Print())

	newNode15 := tree.InsertAfter(node3, NewNode(StringKey("15"), StringValue{"값-15"}, true))
	nodes = append(nodes, newNode15)
	fmt.Println("\n=== 3번 노드 뒤에 15번 노드 삽입 ===")
	fmt.Println(tree.Print())

	newNode16 := tree.InsertAfter(nodes[10], NewNode(StringKey("16"), StringValue{"값-16"}, true))
