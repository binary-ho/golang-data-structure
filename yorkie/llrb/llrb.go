...
func (tree *Tree[K, V]) Put(k K, v V) V {
	tree.root = tree.put(tree.root, k, v)
...
func (tree *Tree[K, V]) Floor(key K) (K, V) {
	node := tree.root
...
func (tree *Tree[K, V]) put(node *Node[K, V], key K, value V) *Node[K, V] {
	if node == nil {
		tree.size++
...
func (tree *Tree[K, V]) remove(node *Node[K, V], key K) *Node[K, V] {
